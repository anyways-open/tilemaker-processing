...
# generate tiles.
mkdir out
docker run -v ${ROOT}:/in -v ${ROOT}/out:/out -v ${ROOT}/coastline/:/coastline -v ${ROOT}/landcover/:/landcover -v ${ROOT}/config/:/config -i --rm anywaysopen/tilemaker:staging /in/belgium-and-neighbourhood-bbox-latest.osm.pbf --output=/out/openmaptiles.mbtiles --process=/config/process-openmaptiles.lua
